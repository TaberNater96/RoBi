...
class ArXivClimateCrawler:
    ...
    def __init__(self):
        ...
        self.rate_limit = 4  # seconds
        self.climate_urls = set()
        self.non_climate_urls = set()
        self.df = pd.DataFrame(columns=['climate_change_urls', 'other_urls'])
        self.topics = {
    ...
    async def crawl_topic(
        ...
        # End the algorithm once 1,000 climate change articles have been found
        while current <= end and len(self.climate_urls) < 1000:
            ...
            for i in range(1, 15_000): # around 15,000 articles are submitted every month
                ...
                if html:
                    ...
                    if self.is_climate_related(title, abstract, topic):
                        ...
                        self.update_dataframe(url, is_climate=True)
                    else:
                        self.non_climate_urls.add(url)
                        self.update_dataframe(url, is_climate=False)
                        
                    self.print_progress()
                
                # Failsafe to ensure loop breaks after 1000 articles
                if len(self.climate_urls) >= 1000:
                    break
    ...
    def update_dataframe(
        ...
        """
        Updates the dataframe with a new row containing the given URL, categorizing it as climate-related or not. All URLs
        that are crawled will be saved and categorized.

        Args:
            url (str): The URL to be added to the dataframe.
            is_climate (bool): Flag indicating whether the URL is related to climate change.

        Returns:
            None: This method modifies the dataframe in-place.
        """
        new_row = {'climate_change_urls': url if is_climate else None,
                   'other_urls': url if not is_climate else None}
    ...
    def print_progress(self) -> None:
        ...
        print(f"\rClimate Change Articles Found: {len(self.climate_urls)} | "
              f"Non-Climate Change Articles Found: {len(self.non_climate_urls)}", end='', flush=True) # flush: print real time
